...
func fg_attempt(fg_ball_turn):
	...
	$TopTd.hide()
	can_glow = false
	#$ThemeControl.apply_fg_background(fg_ball_turn)
...
func pause_game():
	...
	$GameCamera.camera_action_move($GameCamera.CameraPosition.SNAP_DEFAULT)
	...
	if current_ball.player_moved or current_ball.ai_moved:
		...
		match current_ball.current_turn:
			current_ball.Turn.BOT:
				...
				if _local_position.y < 180 and _local_position.y > 100 and _local_velocity_y < 420 and _local_velocity_y > 2:
					if _local_position.x >= 241 and _local_position.x <= 479:
			...
			current_ball.Turn.TOP, current_ball.Turn.AI:
				...
				if _local_position.y > 1100 and _local_position.y < 1180 and _local_velocity.y < 420 and _local_velocity.y > 2:
					if _local_position.x >= 241 and _local_position.x <= 479:
...
# related to IAP, unlocking and locking // ended up not using any of this logic
...
func lock_game():
	...
	#logic to go here if the user reinstalls  and  hasn't purchased
