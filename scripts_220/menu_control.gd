...
onready var TitleAnimation = Main.get_node("MenuLayer/TitleScreen/TitleAnimation")
...
onready var Tournament = Main.get_node("MenuLayer/MainMenu/Tournament")
onready var TournamentButton = Main.get_node("MenuLayer/MainMenu/Tournament/Tournament")
...
onready var Team1PointerOffset = Main.get_node("MenuLayer/MainMenu/SettingsCanvas/SettingsMenu/Team1PointerOffset")
onready var GoogleOn = Main.get_node("MenuLayer/MainMenu/SettingsCanvas/SettingsMenu/GoogleOn")
onready var GameplayMusicOn = Main.get_node("MenuLayer/MainMenu/SettingsCanvas/SettingsMenu/GameplayMusicOn")
...
onready var SmallScreen = Main.get_node("MenuLayer/MainMenu/SettingsCanvas/SettingsMenu/SmallScreen")
...
onready var Rematch = Main.get_node("MenuLayer/GameOver/Rematch")
onready var TournamentBracket = Main.get_node("MenuLayer/GameOver/TournamentBracket")
onready var TournamentBracketText = Main.get_node("MenuLayer/GameOver/TournamentBracket/Text")
onready var Rating = Main.get_node("MenuLayer/GameOver/Rating")
...
onready var PauseHowToPlayBack = Main.get_node("MenuLayer/PauseMenu/CanvasPause/PauseHowToPlayBack")
onready var PauseHowToPlayLabel = Main.get_node("MenuLayer/PauseMenu/CanvasPause/PauseHowToPlayLabel")
onready var GotIt = Main.get_node("MenuLayer/PauseMenu/CanvasPause/GotIt")
onready var PauseHowToPlay= Main.get_node("MenuLayer/PauseMenu/CanvasPause/PauseHowToPlay")
onready var PracticeStats = Main.get_node("MenuLayer/Practice/PracticeStats")
onready var PracticeExit = Main.get_node("MenuLayer/Practice/PracticeExit")
onready var PracticeChallengeMusic = Main.get_node("MenuLayer/PracticeChallengeMusic")
...
onready var RefCamTBTop = Main.get_node("MenuLayer/RefCam/RefCam/RefCamTBTop")
onready var RefCamTBBot = Main.get_node("MenuLayer/RefCam/RefCam/RefCamTBBot")
...
func _ready():
	...
	MenuBackground.position.y = 0
	MenuBackground2.position.y = 3540
	can_scroll = true
	if SaveSettings.reset_times > 0:
		Main.get_node("MenuLayer/MenuSignalControl")._on_ZoomGameIntro_pressed()
...
func _process(_delta):
	if can_scroll:
		if MenuBackground.is_visible():
			if MenuBackground.position.y <= -1771:
				MenuBackground.position.y = 5309
			if MenuBackground2.position.y <= -1771:
				MenuBackground2.position.y = 5309
		MenuBackground.position.y += menu_movement
		MenuBackground2.position.y += menu_movement


func show_play():
	TitleAnimation.play("TitleArcade")
	BlockInput.show()
...
# title music can loop from various points using this when reset back to main menu
func decide_title_music():
	randomize()
	var _selection = int(rand_range(1,4))
	if SaveSettings.reset_times > 1:
		match _selection:
			1:
				TitleMusic.play(0)
			2:
				TitleMusic.play(29.217)
			3:
				TitleMusic.play(69.926)
	else:
		TitleMusic.play()
...
func show_main_menu():
	...
	var tournament_start = Tournament.position
	var tournament_end = Vector2(0,0)
	...
	MainMenuTween.interpolate_property(Tournament, "position", tournament_start, tournament_end, .5, Tween.TRANS_EXPO, Tween.EASE_OUT)
...
func hide_main_menu():
	...
	var tournament_start = Tournament.position
	var tournament_end = Vector2(-1080,0)
	var start_ai_start = StartAI.position
	...
	MainMenuTween.interpolate_property(Tournament, "position", tournament_start, tournament_end, .25, Tween.TRANS_BACK, Tween.EASE_IN)
...
func show_settings():
		...
		match SaveSettings.pixel_push_save_data["is_red_pointer_offset"]:
			true: Team1PointerOffset.set_pressed(true)
			false: Team1PointerOffset.set_pressed(false)
		...
		match SaveSettings.pixel_push_save_data["is_small_screen"]:
			true: SmallScreen.set_pressed(true)
			false: SmallScreen.set_pressed(false)
		match SaveSettings.pixel_push_save_data["gameplay_music"]:
			true: GameplayMusicOn.set_pressed(true)
			false: GameplayMusicOn.set_pressed(false)
...
func get_and_save_settings():
	...
	match Team1PointerOffset.is_pressed():
	...
	if SmallScreen.is_pressed():
		SaveSettings.pixel_push_save_data["is_small_screen"] = true
	if !SmallScreen.is_pressed():
		SaveSettings.pixel_push_save_data["is_small_screen"] = false
	match GoogleOn.is_pressed():
		...
		false:
			GPGS.gpgs_sign_out()
			SaveSettings.pixel_push_save_data["is_google_play"] = false
	match GameplayMusicOn.is_pressed():
		true:
			if Main.current_game_type != Main.GameType.NONE and Main.current_game_type != Main.GameType.PRACTICE and Main.current_game_type != Main.GameType.CHALLENGE:
				Main.play_gameplay_music()
			SaveSettings.pixel_push_save_data["gameplay_music"] = true
		false:
			Main.stop_gameplay_music()
			SaveSettings.pixel_push_save_data["gameplay_music"] = false
...
func show_game_over(winning_player):
	...
	match winning_player:
		1:
			...
			MainMenuReturn.set("custom_styles/hover", _mmr_style)
			MainMenuReturn.set("custom_styles/pressed", _mmr_style)
			Rematch.set("custom_styles/normal", _mmr_style)
			Rematch.set("custom_styles/hover", _mmr_style)
			Rematch.set("custom_styles/pressed", _mmr_style)
			TournamentBracket.set("custom_styles/normal", _mmr_style)
			TournamentBracket.set("custom_styles/hover", _mmr_style)
			TournamentBracket.set("custom_styles/pressed", _mmr_style)
			match ThemeControl.bot_text:
				"RED":
					...
					TournamentBracketText.set("custom_colors/font_color", Color(1, 1, 1))
				"BLUE":
				...
				"YELLOW":
					...
					BotWins.modulate = ThemeControl.bot_color_code
					MainMenuReturn.set("custom_colors/font_color", Color(0, 0, 0))
					Rematch.set("custom_colors/font_color", Color(0, 0, 0))
					TournamentBracketText.set("custom_colors/font_color", Color(0, 0, 0))
				"PINK":
	...
	var main_menu_return_end = Vector2(370,995)
	var rating_start = Rating.rect_position
	var rating_end = Vector2(85,912)
	var tournament_start = TournamentBracket.rect_position
	var tournament_end = Vector2(160, 995)
	var rematch_start = Rematch.rect_position
	var rematch_end = Vector2(50,995)
	var top_stats_start = TopStats.rect_position
	var top_stats_end = Vector2(619,235)
	var bot_stats_start = BotStats.rect_position
	var bot_stats_end = Vector2(529,235)
	var stat_box_start = StatBox.rect_position
	var stat_box_end = Vector2(25,235)
	...
	GameOverTween.interpolate_property(Rematch, "rect_position", rematch_start, rematch_end, 2, Tween.TRANS_EXPO, Tween.EASE_OUT)
	GameOverTween.interpolate_property(Rating, "rect_position", rating_start, rating_end, 2, Tween.TRANS_EXPO, Tween.EASE_OUT)
	GameOverTween.interpolate_property(TournamentBracket, "rect_position", tournament_start, tournament_end, 2, Tween.TRANS_EXPO, Tween.EASE_OUT)
...
func hide_game_over():
	...
	BotWins.hide()
	Rating.hide()
	MainMenuReturn.rect_position = Vector2(370,2200)
	TournamentBracket.rect_position = Vector2(160,2200)
	Rematch.rect_position = Vector2(50,2200)
	MainMenuReturn.set_disabled(true)
	Rematch.set_disabled(true)
	TournamentBracket.set_disabled(true)
	BotWins.position = Vector2(360,-800)
	TopWins.position = Vector2(360,-800)
	BotStats.rect_position = Vector2(2350,260)
	TopStats.rect_position = Vector2(2550,260)
	StatBox.rect_position = Vector2(-1940,260)
	Rating.rect_position = Vector2(85,2271)
...
func show_score():
	...
	var top_end = Vector2(360, -6)
	var bot_end = Vector2(360,1285)
...
func fade_score(direction):
	match direction:
		...
		2:
			BotScoreboard.modulate = Color(1, 1, 1, 0.25)
			TopScoreboard.modulate = Color(1, 1, 1, 0.25)
...
func show_pause_how_to_play():
	PauseHowToPlayBack.show()
	PauseHowToPlayLabel.show()
	GotIt.show()
	ResumeButton.hide()
	QuitMainMenu.hide()
	PauseSettings.hide()
...
func hide_pause_how_to_play():
	PauseHowToPlayBack.hide()
	PauseHowToPlayLabel.hide()
	GotIt.hide()
	ResumeButton.show()
	QuitMainMenu.show()
	PauseSettings.show()
...
func notify_splash_reset():
	var _bot_end = Vector2(1620, 980)
	var _top_end = Vector2(900, -300)
...
func field_goal_ref_cam():
	match Main.current_game_state:
		Main.GameState.FGATTEMPT:
			show_ref_cam()
		Main.GameState.XTRAPT1:
			show_ref_cam()
		_: pass
...
func show_ref_cam():
	match Main.current_game_type:
		Main.GameType.VS_2P, Main.GameType.VS_AI:
	...
	if is_ref_alive:
		match result:
			...
			2:
				...
				RefCamTBTop.hide()
			4:
				RefAnimation.play("RefTBTop")
				RefCamGood.hide()
				RefCamBad.hide()
				RefCamTBBot.hide()
				RefCamTBTop.show()
			3:
				RefAnimation.play("RefTBBot")
				RefCamGood.hide()
				RefCamBad.hide()
				RefCamTBBot.show()
				RefCamTBTop.hide()
...
func show_direction_instruction():
	...
	var _left_direction_end = Vector2(150,637)
	var _right_direction_start = RightDirection.position
	var _right_direction_end = Vector2(570,637)
	match _current_turn:
...
func show_fgf_start():
	...
	FgfStart.show()
	match GPGS.is_player_signed_in:
		true: FgfShowLeaderboard.show()
		false: FgfShowLeaderboard.hide()
...
func fade_in(stream_player):
	AudioTween.interpolate_property(stream_player, "volume_db", -15, 0, 1.25, Transition_type, Tween.EASE_IN, 0)
	AudioTween.start()
